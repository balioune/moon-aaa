...
public class ClaimBuilder {
    ...
    private MoonAuthn moon = new MoonAuthn(); //moon proxy
    ...
    public Claim build(String username, String password, String domain) {
        ...
        JSONObject response = this.moon.keystoneAuthenticate(username, password, domain);
        try {
            if (response.getJSONObject("access")!=null){
                ...
                for (int i=0; i< response.getJSONObject("access").getJSONObject("user").getJSONArray("roles").length(); i++){
                    UserRoles.add(response.getJSONObject("access").getJSONObject("user").getJSONArray("roles").getJSONObject(i).getString("name"));
                }
                ...
                claim.setRoles(ImmutableSet.<String>builder().addAll(UserRoles).build());
                return claim;
            }
        }catch (JSONException e){
            throw new IllegalStateException("Authentication Error : "+ response.getJSONObject("error").getString("title"));
        }
        return claim;
    }
    ...
    protected static class MoonClaim implements Claim, Serializable {
        ...
        public String toString() {
            return "clientId:" + clientId + "," + "userId:" + userId + "," + "userName:" + user
                    + "," + "domain:" + domain + "," + "roles:" + roles + "," + "KeystoneToken: " + keystoneToken;
        }
    }
}
