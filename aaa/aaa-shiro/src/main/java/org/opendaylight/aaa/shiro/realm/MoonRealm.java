...
public class MoonRealm extends AuthorizingRealm{
    ...
    protected AuthenticationInfo doGetAuthenticationInfo(AuthenticationToken authenticationToken) throws AuthenticationException {
        ...
        final MoonPrincipal moonPrincipal = keystoneAuthenticate(username,password,domain);
    ...
    public MoonPrincipal keystoneAuthenticate(String username, String password, String domain){
        ...
        try {
            if (object.getJSONObject("access")!=null){
                String token = object.getJSONObject("access").getJSONObject("token").getString("id");
                String userID = object.getJSONObject("access").getJSONObject("user").getString("id");
                for (int i=0; i< object.getJSONObject("access").getJSONObject("user").getJSONArray("roles").length(); i++){
                    UserRoles.add(object.getJSONObject("access").getJSONObject("user").getJSONArray("roles").getJSONObject(i).getString("name"));
                }
